...
def do_coco_evaluation(
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            # 2019.04.29 add for print per class mAP
    ...
    for key, item in results.results.items():
        msg = ""
        if isinstance(item, str): # iou_type(bbox or segm)
            print(item)
        else:  # dict of result
            for cls, d in item.items():
                if isinstance(d, dict):         # 'AP', 'AP50', 'AP75' per class
                    # for m, values in d.items():
                    #     print("%.3f" % values)
                    msg += "%.3f " % d['AP50']
                else:                           # 'AP', 'AP50', 'AP75' overall
                    msg += "%.3f " % d
        print(msg)
    for key, item in results.results.items():
        msg = ""
...
class COCOResults(object):
    ...
    def update(self, coco_eval):
        ...
        if len(catIds) is 1:
            res[CLASSES[catIds[0]]] = {}
            for idx, metric in enumerate(metrics):
                res[CLASSES[catIds[0]]][metric] = s[idx]
        else:
            for idx, metric in enumerate(metrics):
                res[metric] = s[idx]
