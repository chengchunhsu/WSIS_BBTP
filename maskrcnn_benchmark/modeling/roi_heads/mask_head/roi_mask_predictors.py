...
class MaskRCNNC4Predictor(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = self.mask_fcn_logits(x)

        # MaxPool for per col/row
        x_c = self.maxpool_col(mask)
        x_c = x_c.view(x.size(0), self.n_class, -1)
        x_r = self.maxpool_row(mask)
        x_r = x_r.view(x.size(0), self.n_class, -1)
        x = torch.cat([x_c, x_r], dim=2)
